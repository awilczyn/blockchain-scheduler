...
import static blockchain.util.BIUtil.isLessThan;
import static blockchain.util.ByteUtil.bigIntegerToBytes;
...
public class ECKey implements Serializable {
    ...
    public static byte[] computeAddress(byte[] pubBytes) {
        return applyKeccak(applyKeccak(pubBytes));
    }
    ...
    public static boolean verifyWithRecovery(byte[] data, ECDSASignature signature) {
        try {
            byte[] pubKey = signatureToKeyBytes(applyKeccak(data), signature);
            return verify(applyKeccak(data), signature, pubKey);
